...
let matches = [{id : 1 , season : 2015, winner : 'Hyderabad'},
               ...
               {id : 3 , season : 2016, winner : 'Hyderabad'}]
let deliveries = [
...
test("should return {2015:3} when supplied with 3 matches data for season as 2015",()=>{
    expect(core.matchesPlayedPerYear(matches)).toStrictEqual({"2015":2,"2016":1})
});
...
test("Matches won per team - should throw an error if called with Empty matches data",()=>{
    expect(()=>{ core.matchesWonPerTeam([]) } ).toThrow("No Matches Data Passed");
})
...
test('Testing Extra runs conceded output',()=>{
    expect(()=>{core.runsConcededPerTeam()}).toThrow("No Matches , No deliveries Data ")
    })


test("should throw an error if called without an arg", () => {
    expect( ()=> {core.topTenBowlers()}).toThrow("No Matches , No Deliveries Data Passed")
  })
